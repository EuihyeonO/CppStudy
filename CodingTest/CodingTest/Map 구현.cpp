...
public:
	...
	void Delete(const KeyType& _Key)
	{
		...
		//삭제할 노드의 자식이 없다면
		if (LeftChild == NilNode && RightChild == NilNode)
		{
			//그냥 지운다. 부모노드의 자식에 닐노드를 대입.
		}
		//자식이 1개인데, 왼쪽 노드일 경우.
		else if (LeftChild != NilNode && RightChild == NilNode)
		{
			//왼쪽 노드가 후임자가 된다.
		}
		//자식이 1개인데, 오른쪽 노드일 경우.
		...
		//자식이 2개인 경우.
		else if (LeftChild != NilNode && RightChild != NilNod)
		{
			//삭제하려는 노드가 루트노드의 왼쪽 서브트리에 있다면
			//삭제하려는 노드의 오른쪽 서브트리에서 최소값을 가지는 노드가 후임자가 된다.

			//삭제하려는 노드가 루트노드의 오른쪽 서브트리에 있다면
			//삭제하려는 노드의 왼쪽 서브트리에서 최대값을 가지는 노드가 후임자가 된다.
		}

		/*
		* if_1) 후임자 노드의 색이 Red 라면, 어떠한 속성도 위반하지 않는다.
		* if_2) 후임자 노드의 색이 Black 이라면, Double-Black 발생 -> 해결해야함.
		*/
